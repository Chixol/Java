...
class ExampleClass {
	...
	// 유형 (2가지)
	// 인스턴스 변수: 각 인스턴스가 독립적으로 가질 수 있는 속성
	// 특징: 반드시 인스턴스가 생성된 후에 인스턴스로 사용할 수 있음
	int instanceVariable; //(데이터타입) (변수)
	
	// 클래스 변수: 같은 클래스로 생성된 인스턴스가 공유하는 속성
	// static 키워드를 사용하여 선언
	// 특징: 클래스로 직접 사용할 수 있고 인스턴스에서도 사용할 수 있음
	static int staticVariavle; //static (데이터타입) (변수)
	...
	//      2.일반적으로(100%는 아님) 인스턴스 변수를 사용하는 기능을 구현할 때 사용
	void instanceMethod () {
	...
	// static 키워드 사용하여 선언
	// 특징: 1.인스턴스 변수를 사용할 수 없다 / 일반적으로 인스턴스 변수를 사용하지 않는 기능을 구현할 때 사용
	//      2.클래스로 직접 사용할 수 있고 인스턴스로도 사용할 수 있다
