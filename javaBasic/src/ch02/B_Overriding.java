...
// 규칙
// 1. 상위 클래스에서 선언된 메서드와 메서드 이름, 매개변수의 조합이 같아야 한다.
...
class SuperClass1 {
	...
	SuperClass1 (int superInstanceVariavle) {
		this.superInstanceVariable = superInstanceVariavle;
	}
	void instanceMathod () {
		System.out.println("조상 클래스의 메서드 입니다.");
	}
...
//	void instanceMathod () {
//		System.out.println("SubClass1의 인스턴스 메서드 입니다.");
//	}
	
	// SuperClass1을 상송한 상태에서는 아래 메서드를 가지고 있음
	// void instanceMethod () {...}
	...
	// 부모 클래스에서 기본 생성자가 없다면 자식 클래스의 생성자에서 반드시 부모 클래스의 생성자를 호출해야함
	SubClass1 () {
...
class SubClass2 extends SuperClass1 {
	...
	// 가독성 향상, 개발자의 실수를 방지할 수 있음
	// private 오버라이드 안된다!
	@Override
	void instanceMathod () {
		// super 키워드: 자손 클래스에서 조상클래스의 필드 혹은 메서드에 접근할 때 사용
		super.instanceMathod();
		System.out.println("SubClass2의 인스턴스 메서드 입니다.");
	}
...
public class B_Overriding {
	...
	public static void main(String[] args) {
		...
		//instance1.instanceMathod();
		//instance2.instanceMathod();
		instance3.instanceMathod();
